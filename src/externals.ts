...
function internalsFilter(externals: string[]): (id: string) => true | undefined {
    return (id: string) => {
...
const companion = internalsFilter(
    ['companion', 'external-app', 'image', 'local-storage', 'peer', 'secure-exchange', 'settings'].concat(common)
);
