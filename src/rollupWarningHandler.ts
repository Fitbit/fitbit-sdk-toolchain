...
import { DiagnosticCategory, DiagnosticMessage, logDiagnosticToConsole, DiagnosticHandler } from './diagnostics';
...
function relativeId(id: string): string {
    if (typeof process === 'undefined' || !/^(?:\/|(?:[A-Za-z]:)?[\\|/])/.test(id)) {
        return id;
    }
    return relative(process.cwd(), id);
}

function defaultFormatter(w: RollupWarning): string {
    return w.loc ? `${relativeId(w.loc.file)} (${w.loc.line}:${w.loc.column}) ${w.message!}` : w.message!;
}

const rollupWarningToDiagnostic = (codeCategories: CodeCategoryMap) => (warning: RollupWarning | string) => {
