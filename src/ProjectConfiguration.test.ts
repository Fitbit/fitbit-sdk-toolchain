...
it.each([['a number', 3.14], ['an array', ['foo', 'bar']], ['null', null], ['a boolean', true]])(
...
it('validates the length of the localized display name', () => {
    ...
    expect(config.validateLocaleDisplayName(configFile, 'fr').diagnostics[0]).toEqual(
        expect.objectContaining({
            category: DiagnosticCategory.Error,
            // tslint:disable-next-line:max-line-length
...
const LOCALIZED_DISPLAY_NAME_FOR_FRENCH_STRING = 'Localized display name for French must not be blank';
...
it('validates the localized app display name is not empty', () => {
    ...
    expect(config.validateLocaleDisplayName(configFile, 'fr').diagnostics[0]).toEqual(
        expect.objectContaining({
            category: DiagnosticCategory.Error,
            messageText: LOCALIZED_DISPLAY_NAME_FOR_FRENCH_STRING,
        })
    );
});
...
describe('normalizeProjectConfig', () => {
    it.each([['a number', 3.14], ['an array', ['foo', 'bar']], ['null', null], ['a boolean', true], ['a string', 'fail']])(
        'throws a TypeError if the config JSON root is %s',
        (_, configData) => {
            expect(() => config.normalizeProjectConfig(configData)).toThrow(TypeError);
        }
    );

    it.each([['a number', 3.14], ['an object', { foo: 'bar' }], ['null', null], ['a boolean', true], ['a string', 'fail']])(
        'throws a TypeError if requestedPermissions is %s',
        (_, requestedPermissions) => {
            expect(() => config.normalizeProjectConfig({ fitbit: { requestedPermissions } })).toThrow(TypeError);
        }
    );
