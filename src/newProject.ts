...
import { AppType, normalizeProjectConfig, validateDisplayName } from './ProjectConfiguration';
 ...
 */
function scaffoldDirectory(scaffold: string): void {
    const scaffoldPath = path.join(__dirname, '..', 'scaffold', scaffold);
    fsExtra.copySync(scaffoldPath, scaffold);
}

export default async function newProject(): Promise<void> {
    ...
    const defaultDisplayName = typeof packageName === 'string' ? lodash.startCase(packageName) : undefined;

    const { appType, appDisplayName, withCompanion, withSettings, enabledBuildTargets }: ProjectCreationArgs = await inquirer.prompt<
        ProjectCreationArgs
    >([
        {
            name: 'appType',
