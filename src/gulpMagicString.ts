...
async function mergeSourceMaps(inMap: RawSourceMap, outMap: RawSourceMap): Promise<RawSourceMap> {
...
export default function gulpMagicString(transformerFunc: (code: string, magicString: MagicString) => void): Transform {
    return new Transform({
        ...
        transform(file: Vinyl, _: unknown, next: NextTransformCallback): void {
            ...
            if (!file.isBuffer()) {
                next(
                    new PluginError(PLUGIN_NAME, file.isStream() ? 'Streaming mode is not supported.' : 'Internal error processing file.', {
