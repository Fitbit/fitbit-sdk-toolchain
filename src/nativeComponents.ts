...
export interface INativeComponents {
    buildId: string;
    existingDeviceComponents: stream.PassThrough;
}

function checkBufferLength(buffer: Buffer, expectedLength: number, name: string): void {
    if (buffer.length !== expectedLength) {
        throw new PluginError(PLUGIN_NAME, `${name} must be ${expectedLength} bytes, found ${buffer.length} in ${buffer}`);
    }
}

function formatUUID(buffer: Buffer): string {
    const uuid = buffer.toString('hex');
    return [uuid.substr(0, 8), uuid.substr(8, 4), uuid.substr(12, 4), uuid.substr(16, 4), uuid.substr(20, 12)].join('-');
}

function readMetadata(elfPath: string): IMetadata {
    ...
    function findSection(name: string): Buffer {
        ...
        if (sections.length > 1) {
            throw new PluginError(PLUGIN_NAME, `ELF section '${name}' is present more than once`, { fileName: elfPath });
        }
    ...
    try {
        ...
        throw new PluginError(PLUGIN_NAME, `Could not parse platform specification in .appplatform section: ${ex.message}`, {
            fileName: elfPath,
        });
    }

    if (!Array.isArray(platform)) {
        throw new PluginError(PLUGIN_NAME, `Platform specification should be an array, but found a ${typeof platform}`, {
            fileName: elfPath,
        });
    }
...
function formatComponentAppIdString(path: string, family: string, appID: string): string {
    return `${path} (${family}) has appID ${appID}`;
}

function formatComponentBuildIdString(path: string, family: string, buildID: string): string {
    return `${path} (${family}) has buildID ${buildID}`;
}

export default function nativeComponents(appID: string, componentPaths: string[]): INativeComponents {
    ...
    ${divergentAppIDComponents.map(({ path, family, appID }) => formatComponentAppIdString(path, family, appID)).join('\n  ')}`
        );
    }
    ...
    ${components.map(({ path, family, buildID }) => formatComponentBuildIdString(path, family, buildID)).join('\n  ')}`
        );
    }
