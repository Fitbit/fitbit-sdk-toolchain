...
import ProjectConfiguration, { AppType, ClockProjectConfiguration } from './ProjectConfiguration';
...
function expectPackageManifest(stream: Readable, projectConfig: ProjectConfiguration): jest.Matchers<Promise<unknown>> {
    return expect(
...
it('builds a package manifest with a companion', () => expectValidPackageManifest({ hasCompanion: true }).toMatchSnapshot());
...
it('emits an error if both JS and native device components are present', () => {
    ...
    return expectPackageManifest(stream, projectConfig).rejects.toThrowErrorMatchingSnapshot();
});
...
it('builds a package manifest with a native device component', () => expectValidPackageManifest({ nativeApp: true }).toMatchSnapshot());
