...
function expectTranslationsForLanguages(done: jest.DoneCallback, ...languages: string[]): Writable {
    const expected = new Set(languages);
    const actual = new Set<string>();

    return new Writable({
        objectMode: true,
        write(file: Vinyl, _: unknown, next: (error?: Error | null) => void): void {
...
function translationFileSource(...paths: string[]): PassThrough {
...
function compileExpectError(done: jest.DoneCallback): Transform {
...
describe('rejects .po files whose names are not acceptable language tags', () => {
    it.each(['e.po', 'en-USA.po', 'sl-IT-nedis.po'])('%s', (path: string, done: jest.DoneCallback) => {
