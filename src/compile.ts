...
import { DiagnosticCategory, logDiagnosticToConsole, DiagnosticHandler } from './diagnostics';
...
function pluginIf(condition: boolean, plugin: () => rollup.Plugin): rollup.Plugin[] {
...
export default function compile(
    ...
    input: string,
    output: string,
    { allowUnknownExternals = false, onDiagnostic = logDiagnosticToConsole }: ICompileFileOptions
): pumpify {
        ...
        rollupToVinyl(
            output,
            {
                ...
                plugins: [
                    ...
                    ...pluginIf(sdkVersion().major < 3 || component === ComponentType.SETTINGS, resourceImports),
                ...
                onwarn: rollupWarningHandler({
                    onDiagnostic,
                    codeCategories: allowUnknownExternals ? { UNRESOLVED_IMPORT: DiagnosticCategory.Warning } : undefined,
                }),
            },
            {
                format: 'cjs',
