...
export function isPluginError(value: unknown): value is PluginError {
    ...
    return typeof showStack === 'boolean' && typeof plugin === 'string' && toString !== Error.prototype.toString;
}
...
function hasEnumerableProp<T, K extends string | number | symbol>(obj: { [P in K]?: T }, prop: K): obj is { [P in K]: T } {
...
function convertToDiagnostic(error: PluginError<{ columnNumber?: number }>): Diagnostic {
        ...
        if (hasEnumerableProp(error, 'lineNumber')) {
            diagnostic.file.position = {
                start: {
                    ...
                    character: hasEnumerableProp(error, 'columnNumber') ? error.columnNumber - 1 : undefined,
                },
            };
        }
    }
