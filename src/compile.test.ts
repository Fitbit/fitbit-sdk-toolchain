...
import compile, { ICompileFileOptions } from './compile';
...
const IMPORT_IMAGE_FILE_NAME = 'importImage.js';
...
beforeEach(() => {
    ...
    jest.spyOn(ts, 'findConfigFile').mockReturnValue(testResourcePath('tsconfig.json'));
});

function testResourcePath(...paths: string[]): string {
    return path.resolve(__dirname, '__test__', 'compile', ...paths);
}

function compileFile(filename: string, compileOptions: ICompileFileOptions = {}): Promise<Vinyl> {
    compileOptions.component = compileOptions.component || ComponentType.COMPANION;
    const allowUnknownExternals = compileOptions.allowUnknownExternals;

    return getFileFromStream(
        compile(compileOptions.component, testResourcePath(filename), 'output.js', {
...
])('build passes when %s', (_, filename) => expect(compileFile(filename).then(getVinylContents)).resolves.toMatchSnapshot());

// Just check build is successful
it.each([['importing a package', 'importPackage.js']])('build passes when %s', (_, filename) =>
...
it('allows importing image files when building settings', () =>
    expect(compileFile(IMPORT_IMAGE_FILE_NAME, { component: ComponentType.SETTINGS }).then(getVinylContents)).resolves.toMatchSnapshot());

describe('when targeting SDK 1.0', () => {
    beforeEach(() => mockSDKVersion.mockReturnValue({ major: 1, minor: 0 }));

    it('allows JSON imports', () => expect(compileFile('importJSON.js').then(getVinylContents)).resolves.toMatchSnapshot());

    it('allows unintentionally non-relative imports', async () => {
        await expect(compileFile('incorrectlyNonRelativeImport.js').then(getVinylContents)).resolves.toMatchSnapshot();
        expect(mockDiagnosticHandler.mock.calls[0]).toMatchSnapshot();
    });

    it.each([ComponentType.DEVICE, ComponentType.COMPANION])('allows importing image files when building %s', (component: ComponentType) =>
        expect(compileFile(IMPORT_IMAGE_FILE_NAME, { component })).resolves.toBeDefined()
    );
});
...
describe('when targeting SDK 2.0', () => {
    ...
    it('does not allow JSON imports', () =>
        expect(compileFile('importJSON.js').then(getVinylContents)).rejects.toThrowErrorMatchingSnapshot());

    it('does not allow unintentionally non-relative imports', () =>
        expect(compileFile('incorrectlyNonRelativeImport.js').then(getVinylContents)).rejects.toThrowError());
...
describe('when targeting SDK 3.0', () => {
    ...
    it('emits ES6 code', () => expect(compileFile('ES6.js').then(getVinylContents)).resolves.toMatchSnapshot());

    it('emits ES5 code for device', () =>
        expect(compileFile('ES6.js', { component: ComponentType.DEVICE }).then(getVinylContents)).resolves.toMatchSnapshot());

    it.each([ComponentType.DEVICE, ComponentType.COMPANION])(
        'does not allow importing image files when building %s',
        (component: ComponentType) => expect(compileFile(IMPORT_IMAGE_FILE_NAME, { component })).rejects.toBeDefined()
    );
});
...
])('build fails when %s', (_, filename) => expect(compileFile(filename)).rejects.toThrowErrorMatchingSnapshot());

it.each([['JS code with bad syntax', 'badSyntax.js'], ['TS code with a type error', 'typeError.ts']])(
...
describe('when compiling a module with statements above an import declaration', () => {
    ...
    it('emits a sourcemap with the first statement after the import correctly mapped', async () => {
        ...
        expect(map.originalPositionFor({ line: logLine + 1, column: logColumn })).toMatchObject({ line: 3, column: 0 });
    });
});
...
describe('when allowUnknownExternals is enabled', () => {
    it('successfully builds an app with unknown external imports', async () => {
        await expect(
            compileFile('unknownExternalImport.js', { allowUnknownExternals: true }).then(getVinylContents)
