...
export type ProjectConfiguration = AppProjectConfiguration | ClockProjectConfiguration;
...
interface IPermissionTypeDescription {
    name: string;
    description: string;
}
interface IPermissionTypes {
    access_activity: IPermissionTypeDescription;
    access_user_profile: IPermissionTypeDescription;
    access_heart_rate: IPermissionTypeDescription;
    access_location: IPermissionTypeDescription;
    access_internet: IPermissionTypeDescription;
    run_background: IPermissionTypeDescription;
}

interface IRestrictedPermissionTypes {
    fitbit_token: IPermissionTypeDescription;
    external_app_communication: IPermissionTypeDescription;
    access_secure_exchange: IPermissionTypeDescription;
}

const permissionTypes: Readonly<IPermissionTypes> = Object.freeze({
    access_activity: {
        name: 'Activity',
...
const restrictedPermissionTypes: Readonly<IRestrictedPermissionTypes> = Object.freeze({
    fitbit_token: {
...
interface IGetPermissionTypesProps {
    includeRestrictedPermissions: boolean;
}

export function getPermissionTypes({
    includeRestrictedPermissions = false,
}: IGetPermissionTypesProps): IPermissionTypes | IPermissionTypes & IRestrictedPermissionTypes {
...
function constrainedSetDiagnostics({
    ...
    if (unknownValues.length > 0) {
        ...
        const unknownValueOther = lodash.without(unknownValues, ...unknownValueStrings).map(String);
        if (unknownValueStrings.length) {
            const errString = `One or more ${valueTypeNoun} was invalid: ${unknownValueStrings.join(', ')}`;
            if (notFoundIsFatal) {
                diagnostics.pushFatalError(errString);
            } else {
                diagnostics.pushWarning(errString);
            }
        }
    ...
    const duplicatedValues = lodash.uniq(actualValues).filter(value => actualValues.indexOf(value) !== actualValues.lastIndexOf(value));
...
export function validateAppType(config: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
...
// TODO: check so that we don't have a union type here
export function validateDisplayName(name: string): string | boolean {
    if (name.length === 0) {
...
export function validateProjectDisplayName(config: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
...
export function validateWipeColor(config: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
...
export function validateRequestedPermissions({ requestedPermissions }: ProjectConfiguration): DiagnosticList {
...
export function validateBuildTarget({ buildTargets }: ProjectConfiguration): DiagnosticList {
...
export function validateLocaleDisplayName({ i18n }: ProjectConfiguration, localeKey: keyof typeof LOCALES): DiagnosticList {
    const diagnostics = new DiagnosticList();
...
export function validateLocaleDisplayNames(config: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
    for (const localeKey of Object.keys(LOCALES)) {
        diagnostics.extend(validateLocaleDisplayName(config, localeKey as keyof typeof LOCALES));
    }
    return diagnostics;
}

export function validateSupportedLocales({ i18n }: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();

    const unknownLocales = lodash.without(Object.keys(i18n), ...Object.keys(LOCALES));
...
export function validateAppUUID({ appUUID }: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
...
export function validate(config: ProjectConfiguration): DiagnosticList {
    const diagnostics = new DiagnosticList();
    [
        validateAppUUID,
